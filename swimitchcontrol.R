...
# Figure 4:(a) Simulated parasite prevalence in snails without treatment using infection data from
#Higgins Lake, MI. Data includes mean (and one standard error) prevalence across 10 sites at Higgins
#Lake at four summer time points. Horizontal dotted lines are at the ideal and epidemic prevalence
#thresholds defined in Section 3.1. (b) Simulated merganser densities during the residency period
#without praziquantel treatment. Thicker weight on t-axis represents the recreational period.
#pdf("~/OneDrive - University of Wisconsin-La Crosse/SwimmersItch/paper/figures/Figure4a.pdf",width = ((1+sqrt(5))/2)*7)
ggplot(data = y.df) + 
  ...
  theme(legend.position = c(.3, .9),
        ...
        aspect.ratio = .7)
#dev.off()
#pdf("~/OneDrive - University of Wisconsin-La Crosse/SwimmersItch/paper/figures/Figure4b.pdf",width = ((1+sqrt(5))/2)*7)
ggplot(data = y.df,
...
# Figure 5: Parameter sensitivity of the mean density of infected snails during the resident period
# after parameter values are changed by 10%.
...
# Figure 6: (a) Optimal control u(t) with weights Alo and Ahi. (b) Simulated density of infected birds
# when treated. (c) Simulated schistosome prevalence in snails from two treatment cases. Horizontal
# dotted lines represent ideal (0.24%) and epidemic (2%) snail prevalence thresholds. Thicker weight
# on t-axis represents the recreational period.
#pdf("~/OneDrive - University of Wisconsin-La Crosse/SwimmersItch/paper/figures/Figure6a.pdf",width = ((1+sqrt(5))/2)*7)
ggplot(data = ylo.df) + 
...
#pdf("~/OneDrive - University of Wisconsin-La Crosse/SwimmersItch/paper/figures/Figure6b.pdf", width = ((1+sqrt(5))/2)*7)
ggplot(data = y.df) + 
  ...
  theme(legend.position = c(.3, .8),
        ...
        aspect.ratio = .7)
#dev.off()
#pdf("~/OneDrive - University of Wisconsin-La Crosse/SwimmersItch/paper/figures/Figure6c.pdf", width = ((1+sqrt(5))/2)*7)
ggplot(data = y.df) + 
...
# FIGURE 7: (a) Practical optimal control from minimizing the mean density of the infected snail
# population by treating a maximum number of birds M on day t1. The dashed curve is the rst
# day that the bird density at the lake equals the greatest number the lake manager can possibly
# treat. (b) An example of the simulated infected bird density when M = 22 birds are treated on
# day t1 = 13.
notreat <- PracticalModel1(treat1 = 0, max.treat = NA)
NB <- (notreat$SB+notreat$EB+notreat$IB)
time2reach <- numeric(30)
for (m in 1:30){
...
ggplot(data = opt.time1[1:40,],
       aes( x = M, y = day, linetype = "M", show.legend = FALSE))+
  geom_point(show.legend = FALSE) +
  geom_line(data = time2reach.df,
            aes( x = M, y = day, linetype = "min. time to reach M"),show.legend = FALSE)+
  geom_point(size = 3, aes(x = M.opt , y = t.opt)) +
  geom_text(aes(x = M.opt + 2, y = t.opt-0.5, label = "(22,13)"), size=7)+
  scale_linetype_manual(values = c("M"='solid', "min. time to reach M" = 'dashed')) +
  labs(linetype = "Legend",
       x = "Maximum Birds Treated",
       y = "Day Treated") +
  scale_x_continuous(limits = c(0,30.5), breaks = seq(0,31,2), expand = c(0,0)) +
  scale_y_continuous(limits = c(0,23), breaks = seq(0,23,2), expand = c(0,0)) +
  theme_classic() +
...
# Figure 8: (a) Practical optimal control from minimizing the mean density of the infected snail
# population by treating a maximum number of birds M on day t1 (4) and day t2 (5). (b) An
# example of the simulated infected bird density when M = 15 birds are treated on days t1 = 10 and
# t2 = 22.
...
ggplot(data = opt.time2,
       aes( x = M, y = day1, shape = "day 1", color = "day 1" )) +
  geom_point(show.legend = TRUE) +
  geom_point(aes( x = M, y = day2, shape = "day 2", color = "day 2"),show.legend = FALSE)+
  ...
  scale_y_continuous(limits = c(0,24), breaks = seq(0,24,2), expand = c(0,0))+
  theme_classic() +
  theme(
